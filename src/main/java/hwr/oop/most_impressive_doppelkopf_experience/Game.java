...
public class Game implements Serializable {
  ...
  public void handOutCards() {
    ...
    for (int i = 0; i < NUM_PLAYERS; i++) {
      for (int j = NUM_CARDS_PER_PLAYER * i;
          j < NUM_CARDS_PER_PLAYER + NUM_CARDS_PER_PLAYER * i;
          j++) {
  ...
  public boolean handOutCardsAreValid(Player player) {
    ...
    long nineCards =
        player.getHand().stream().filter(card -> card.getSymbol() == CardSymbols.NINE).count();
    if (nineCards >= 5) {
      return false;
    }
    ...
    long kingCards =
        player.getHand().stream().filter(card -> card.getSymbol() == CardSymbols.KING).count();
    if (nineCards >= 4 && kingCards >= 4) {
      return false;
    }
    ...
    for (Card playerCard : player.getHand()) {
      if (playerCard.getSymbol() == CardSymbols.NINE
          && ninesAllColours.stream()
              .noneMatch(card -> card.getColour() == playerCard.getColour())) {
        ninesAllColours.add(playerCard);
      }
    }

    if (ninesAllColours.size() >= 4) {
      return false;
    }

    // weniger oder gleich 2 Trümpfe
    long trumpCards =
        player.getHand().stream().filter(card -> card.getColour() == CardColours.TRUMP).count();
    if (trumpCards <= 2) {
      return false;
    }
    ...
    long trumpCardsHigherJack =
        player.getHand().stream()
            .filter(card -> card.getColour() == CardColours.TRUMP && card.getWorth() > 2)
            .count();
  ...
  public boolean cardFollowsSuit(Card cardToPlay, Player playerWhoPlaysTheCard, Stich stich) {
    if (!stich.getDiscardCards().isEmpty()) {
      ...
      boolean playedCardColourEqualsFirstCardColorOfDiscardPile =
          firstCardColor.equals(cardToPlay.getColour());
      boolean playerHasCardWithFirstCardColorOfDiscardPile =
          playerWhoPlaysTheCard.getHand().stream()
              .anyMatch(obj -> obj.getColour().equals(firstCardColor));

      return playedCardColourEqualsFirstCardColorOfDiscardPile
          || !playerHasCardWithFirstCardColorOfDiscardPile;
    }
  ...
  public void distributeTeams() {
    ...
    players.forEach(
        player -> {
          boolean foundCQ = player.getHand().stream().anyMatch(card -> card.getName().equals(old));
